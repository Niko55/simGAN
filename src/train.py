    ...
    def train(self):
        for e in range(self.EPOCHS):
            ...
            while min(len(X_real_temp) ,len(X_sim_temp) ) >self.BATCH:
                # Keep track of Batches b=b+1

                count_real_images = int(self.BATCH)
                starting_indexs = randint(0, (min(len(X_real_temp),len(X_sim_temp)) -count_real_images))

                real_images_raw = X_real_temp[ starting_indexs: (starting_indxs + count_real_images) ]
                real_images = real_images_raw.reshape( count_real_imags, self.H, self.W, self.C )

                y_real = np.array([[[1.0, 0.0]] * self.discriminator.Discriminator.output_shape[1]] * self.BATCH)

                sim_images_raw = X_sim_temp[ starting_indexs: (starting_indxs + count_real_images) ]
                sim_images = sim_images_raw.reshape( count_real_imags, self.H, self.W, self.C )
